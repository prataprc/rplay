...
use vulkano::{app_info_from_cargo_toml, device::Features};
...
fn main() {
    ...
    if opts.limits && pd.is_some() {
        ...
        print_features(pd, pd.supported_features());
    } else if pd.is_some() {
        print_pd(pd.as_ref().unwrap());
...
fn print_features(pd: &PhysicalDevice, features: &Features) {
    let items = vec![
...
fn show_window() {
    ...
    let (mut window, events) = glfw
        .create_window(800, 300, "Hello this is window", glfw::WindowMode::Windowed)
